...
kx_aux = @(mu_x,m_aux,Lx) (mu_x + 2*m_aux*pi)/Lx;
ky_aux = @(mu_y,n_aux,Ly) (mu_y + 2*n_aux*pi)/Ly;
        
% Wavenumber - Paper - Equation  
% f_kz = @(k_matricial,kx_aux,ky_aux) -(1j)*sqrt(kx_aux.^2+ky_aux.^2-k_matricial.^2);
kz_f = @(k,kx_aux,ky_aux) -(1j)*sqrt(kx_aux^2+ky_aux^2-k^2);
...
for i=1:nfreq
    ...
    % Fluid dynamic stiffness
    Df1mn = zeros(m_index,n_index);
    Df2mn = zeros(m_index,n_index);
    ...
    m_new = m; m_new(round(length(m)/2)) = [];
    n_new = m; n_new(round(length(n)/2)) = [];
    V_mn = zeros(GDof+numberRes, n_index, m_index);
    Df = zeros(GDof+numberRes,GDof+numberRes);
    for cont_m=1:m_index-1
        for cont_n=1:n_index-1
            
            m_aux = m_new(cont_m);
            n_aux = n_new(cont_n);
    
            % Wavenumber in z direction (air) - depending on harmonics - Matricial form
            kx_mn = kx_aux(mu_x,m_aux,Lx);
            ky_mn = ky_aux(mu_y,n_aux,Ly);

            % Calling wavenumber function to calculate kz1mn
            kz1mn = kz_f(k,kx_mn,ky_mn); 
            ...
            %Equation 10
            Df1mn(cont_m,cont_n) = (-1j*rho1*w^2)./(kz1mn);
            Df2mn(cont_m,cont_n) = (-1j*rho2*w^2)./(kz2mn);
            
            % Fluid force for a (m,n) harmonic
            [v1_mn,v1_mn_H] = force_fluid_test(GDof, numberRes,nnode,nelem,elem,nodes,...
    csi_aux,eta_aux,wcsi_aux, weta_aux, dof,kx_mn,ky_mn);
            
            % Storing 
            V_mn(:,cont_n,cont_m) = v1_mn_H;
            
            % Computing the dsm of the fluid
            Df = Df + Df1mn(cont_m,cont_n)*(v1_mn*(v1_mn')) + ...
                Df2mn(cont_m,cont_n)*(v1_mn*(v1_mn'));
        end
    end
...
%     csi_aux,eta_aux, wcsi_aux, weta_aux, dof, Df1mn,kx_aux,ky_aux,kz2mn,h);
    ...
    for cont_m =1:m_index
     for cont_n = 1:n_index
%          V_mn(:,cont_n,cont_m)
         v_aux = V_mn(:,cont_n,cont_m);
...
%          v_aux1(1:dof:GDof) = v_aux;
         ...
         W1_mn(cont_m,cont_n) = (1/(Lx*Ly))*v_aux.'*q_total;
    ...
    for cont_m=1:m_index
        for cont_n=1:n_index
            ...
            % Equation 4
            aux_disp = exp(-1j*(kx_mn*x_disp+ky_mn*y_disp));
            disps_mn = W1_mn(cont_m,cont_n).* aux_disp;
            
            aux_soma = aux_soma + disps_mn;
        end
    end
...
%     disp(i) = sum(sum(disps_mn,2));
    ...
    for cont_m=1:m_index
        for cont_n=1:n_index
            ...
            tau_mn_num= real(tau2_aux.*abs(P2_mn(cont_m,cont_n).^2));
            tau_mn_den = tau1_aux.*abs((P_inc).^2);
